...
const Index = () => {
  ...
  const pengzhuangfn = () => {
    ...
    const useBumpNextTick = () => {
      let canvasDom2 = canvasRef2.current!
      let canvasWidth = 500;
      let canvasHeight = 500;
      canvasDom2.width = canvasWidth
      canvasDom2.height = canvasHeight
      let ctx = canvasDom2.getContext("2d") as any;

      function drawFn() {
        drawArray.forEach((fn: any) => fn().draw(ctx));
      }

      //正方形
      let xyListSourceSquare = [
        [10, 10],
        [10, 50],
        [50, 50],
        [50, 10],
      ...
      function drawSquare() {
        let xyList = xyListSourceSquare

        function draw(ctx: any) {
          ctx.beginPath();
          xyList.forEach((list, listIndex) => {
            ctx[listIndex === 0 ? 'moveTo' : 'lineTo'](list[0], list[1]);
          })
          ctx.fillStyle = "black";
          ctx.closePath();
          ctx.stroke();
        ...
        return { xyListSource: xyListSourceSquare, xyList, name: "正方形", draw }
      }

      //圆形
      let xyListSourceCircle = [
        [100, 100]
      ]

      function drawCircle() {
        let xyList = xyListSourceCircle

        function draw(ctx: any) {
          ctx.beginPath();
          ctx.arc(xyList[0][0], xyList[0][1], 50, 0, 2 * Math.PI);
          ctx.fillStyle = "blue";
          ctx.fill();
          ctx.closePath();
        }

        return { xyListSource: xyListSourceCircle, xyList, name: "圆形", draw }
      }

      // 三角形
      let xyListSourceTriangle = [
        [150, 150],
        [200, 50],
        [250, 150]
      ];
      ...
      function drawTriangle() {
        let xyList = xyListSourceTriangle
        ...
        return { xyListSource: xyListSourceTriangle, xyList, name: "三角形", draw }
      }

      //贝塞尔曲线
      let xyListSourceBezier = [
        [240, 200],
        [110, 110],
        [199, 278],
        [300, 379],
      ...
      function drawBezier() {
        let xyList = xyListSourceBezier

        function draw(ctx: any) {
          ctx.beginPath();
          ctx.moveTo(xyList[0][0], xyList[0][1]);
          ctx.bezierCurveTo(xyList[1][0], xyList[1][1], xyList[2][0], xyList[2][1], xyList[3][0], xyList[3][1]);
          ctx.lineTo(xyList[4][0], xyList[4][1]);
          ctx.closePath();
          ctx.stroke();
        }

        return { xyListSource: xyListSourceBezier, xyList, name: "贝塞尔曲线", draw }
      }

      let drawArray: any = [];
      drawArray.push(drawSquare, drawCircle, drawTriangle, drawBezier);

      function clickDraw(e: any, xyInfo: any) {
        // console.log(xyInfo);
        ctx.clearRect(0, 0, canvasWidth, canvasHeight);
        // 遍历绘制图形
        todoList.map((todoInfo: any, todoInfoIndex: any) => {
          todoInfo.xyListSource.map((f: any, fIndex: any) => {
            f[0] = f[0] + xyInfo.x
            f[1] = f[1] + xyInfo.y
          })
        })
        realDrawFn()
        getOverShapes(drawArray, todoList, (shapeInfo: any) => {
          console.log(shapeInfo && shapeInfo().name);
          let tempName = shapeInfo && shapeInfo().name
          if (tempName) {
            message.warning(`和${tempName}碰撞了！`)
          }
        })
      }

      function realDrawFn() {
        drawArray.forEach((d: any) => {
          d().draw(ctx)
        })
      }

      function checkTodoListFn(evt: any) {
        const canvasInfo = canvasDom2.getBoundingClientRect();
        const x = evt.clientX - canvasInfo.left;
        const y = evt.clientY - canvasInfo.top;
        ctx.clearRect(0, 0, canvasWidth, canvasHeight);
        todoList = []
        // 遍历绘制图形
        for (var i = drawArray.length; i--;) {
          // 每绘制一个图形就判断一次当前鼠标的坐标是否在这个图形上，然后进行自定义操作
          let info = drawArray[i]()
          info.draw(ctx)
      ...
      let mousemoveFnPre: any
      let todoList: any = []
      const mousedownFn = (evt: any) => {
        console.log("mousedown");
        checkTodoListFn(evt)
        mousemoveFnPre = (moveEvt: any) => {
          mousemoveFn(moveEvt, evt)
        }
        canvasDom2.addEventListener('mousemove', mousemoveFnPre, false);
      }

      let preMoveEvt: any = null
      const mousemoveFn = (moveEvt: any, downEvt: any) => {
        // console.log("mousemove");
        if (!preMoveEvt) {
          preMoveEvt = downEvt
        }
        let xyInfo = {
          x: moveEvt.clientX - preMoveEvt.clientX,
          y: moveEvt.clientY - preMoveEvt.clientY
        }
        preMoveEvt = moveEvt
        clickDraw(downEvt, xyInfo)
      }
      const mouseupFn = (evt: any) => {
        console.log("mouseup");
        preMoveEvt = null
        canvasDom2.removeEventListener("mousemove", mousemoveFnPre, false)
      }
      canvasDom2.addEventListener('mousedown', mousedownFn, false);
      canvasDom2.addEventListener('mouseup', mouseupFn, false);


      function getOverShapes(shapeList = [], targetShape: any, cb: any) {
        if (targetShape.length === 0) {
          return
        }
        let overShapeList = shapeList.filter((f: any) => {
          return JSON.stringify(f().name) !== JSON.stringify(targetShape[0].name)
        })


        overShapeList.map((shapeInfo: any) => {
          if (shapeInfo.setTimeOutMark) {
            clearTimeout(shapeInfo.setTimeOutMark)
          }
          shapeInfo.setTimeOutMark = setTimeout(() => {
            let htmlCanvasElement = document.createElement("canvas");
            htmlCanvasElement.width = canvasWidth
            htmlCanvasElement.height = canvasHeight
            let aStr = htmlCanvasElement.toDataURL()
            let _ctx: any = htmlCanvasElement.getContext("2d");
            _ctx.clearRect(0, 0, canvasWidth, canvasHeight);
            _ctx.fillStyle = "#f00"
            shapeInfo().draw(_ctx);
            _ctx.globalCompositeOperation = 'source-in';
            targetShape[0].draw(_ctx, 111);
            let bStr = htmlCanvasElement.toDataURL()
            cb(aStr !== bStr ? shapeInfo : null)
          })
        })
      }
    ...
    <>
      ...
      <div className="canvas-wrap">
        <div className="canvas-div">
          <h2>抽奖笔刷</h2>
          <canvas className="canvasClass" ref={brushDomRef}></canvas>
        </div>
        <div className="canvas-div">
          <h2>碰撞检测</h2>
          <canvas className="canvasClass" ref={canvasRef2}></canvas>
        </div>
      </div>
